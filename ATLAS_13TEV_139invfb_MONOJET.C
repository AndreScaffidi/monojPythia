//
// ---- New 139 invfb analysis: https://arxiv.org/pdf/2102.10874.pdf
//
...
void ATLAS_13TEV_139invfb_MONOJET(const char *inputFile, const char *outFileone) {
  using namespace std;
  ...
  const vector<double> METMINS = {200., 250., 300., 350., 400., 500., 600., 700., 800., 900., 1000.}; // 139 fb^-1 has extra bin
  ...
  for (Int_t entry = 0; entry < treeReader->GetEntries(); ++entry) {
    ...
    //cout << "MET=" << met << endl;
    if (met < 200) continue; // 139invfb


    // Isolate jets from MET
    // NB. Loose selection = no spurious noise = no action needed?
    // New delta phi cut in first met bin
    vector<const Jet*> anajets2;
    for (const Jet* j : anajets) {
      if (met>=200 && met<250 && fabs(j->P4().DeltaPhi(vmet)) > 0.6)  anajets2.push_back(j);
      else if (fabs(j->P4().DeltaPhi(vmet)) > 0.4) anajets2.push_back(j);
    }
    ...
    // Require leading jet pT > 150 GeV, and within |eta| < 2.4
    if (anajets2[0]->PT < 150) continue;
